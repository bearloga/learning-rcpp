...
```{r nbc_example, dependson = 'nbc_source', cache = TRUE}
...
```{Rcpp cerealization, cache = TRUE, eval = FALSE}
// [[Rcpp::plugins(cpp11)]]
...
Unfortunately, it seems there's a typo in **naive_bayes_classifier_impl.hpp**:
...
and in **naive_bayes_classifier.hpp**:
...
```{Rcpp nbc_serialization_source, cache = TRUE, eval = FALSE}
// [[Rcpp::plugins(mlpack11)]]
// [[Rcpp::depends(RcppArmadillo)]]
...
```{r nbc_serialization_example, dependson = 'nbc_serialization_source', cache = TRUE, eval = FALSE}
...
```{Rcpp nbc_deserialization_source, cache = TRUE, eval = FALSE}
// [[Rcpp::export]]
...
```{r nbc_deserialization_example, dependson = c('nbc_serialization_example', 'nbc_deserialization_source'), cache = TRUE, eval = FALSE}
fit_e1071 <- e1071::naiveBayes(training_x, training_y)
# Performance Comparison
microbenchmark(
  `e1071 prediction` = e1071:::predict.naiveBayes(fit_e1071, testing_x, type = "class"),
